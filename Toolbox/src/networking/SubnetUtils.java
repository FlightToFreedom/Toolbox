...
public class SubnetUtils {
	 ...
	 * @since 2.2
	 */
	public void setInclusiveHostCount(
	...
	public final class SubnetInfo {
		...
		private int low() {
			return (isInclusiveHostCount() ? network()
					: broadcast() - network() > 1 ? network() + 1
							: 0);
		}

		private int high() {
			return (isInclusiveHostCount() ? broadcast()
					: broadcast() - network() > 1 ? broadcast() - 1
							: 0);
		}
	...
	private int toInteger(String address) {
		Matcher matcher = addressPattern
				.matcher(address);
		if (matcher.matches()) {
	 ...
	 * exception otherwise.
	 */
	private int rangeCheck(int value, int begin,
			int end) {
	 ...
	 */
	private String toCidrNotation(String addr,
			String mask) {
